    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-2' : 'bg-transparent py-4'}`}>
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center space-x-8">
          {NAVIGATION.map((item) => (
            <a key={item.name} href={item.href} className="text-gray-700 hover:text-green-600 font-medium transition-colors">
              {item.name}
            </a>
          ))}
        </div>
        ...
        <div className="flex items-center space-x-4">
          <a href="tel:1900xxxx" className="hidden lg:flex items-center text-green-700 font-bold">
            <Phone className="w-5 h-5 mr-2" />
            1900xxxx
          </a>
          <button className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-full font-bold shadow-lg transition-transform hover:scale-105 active:scale-95">
            Nhận tư vấn ngay
          </button>
        </div>
      </div>
        ...
        {FEATURES.map((feat, i) => (
          <div key={i} className="bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all group">
            ...
            <div className="p-6">
              ...
              <h4 className="text-xl font-bold text-gray-900 mb-2">{feat.title}</h4>
    ...
    <div className="container mx-auto px-4">
      <div className="flex flex-col lg:flex-row gap-12 items-center">
        ...
        <div className="lg:w-1/2">
          <img src={LIFESTYLE_IMAGES.WORKING} alt="Professional Working" className="rounded-3xl shadow-2xl" />
        </div>
      </div>
    </div>
  </section>
        ...
        {STEPS.map((step, i) => (
          <div key={i} className="relative group">
            ...
            <div className="bg-gray-800 rounded-3xl overflow-hidden h-full flex flex-col border border-gray-700 group-hover:border-green-500 transition-all">
              <img src={step.img} alt={step.title} className="h-40 w-full object-cover grayscale group-hover:grayscale-0 transition-all" />
              <div className="p-6">
      ...
      <div className="mt-20 grid md:grid-cols-2 gap-12 items-center">
        <div className="space-y-6">
          ...
          <div className="bg-green-500/10 p-4 rounded-2xl border border-green-500/20 text-green-300 font-medium">
            Tần suất lý tưởng: Chỉ cần 1-2 lần mỗi ngày để cảm nhận thay đổi rõ rệt.
          </div>
        </div>
        <div className="relative aspect-video rounded-3xl overflow-hidden bg-gray-800 flex items-center justify-center shadow-2xl group cursor-pointer">
          <img src={LIFESTYLE_IMAGES.RELAX} alt="Video Thumbnail" className="absolute inset-0 w-full h-full object-cover opacity-40 group-hover:scale-105 transition-transform" />
          ...
          <div className="absolute bottom-4 left-4 font-bold text-lg">VIDEO HƯỚNG DẪN</div>
        </div>
      </div>
    </div>
  </section>
  ...
  return (
    <div className="border-b border-gray-100 last:border-0">
      ...
      >
        ...
        {isOpen ? <ChevronUp className="text-green-600" /> : <ChevronDown className="text-gray-400 group-hover:text-green-600" />}
      </button>
    ...
    <div className="container mx-auto px-4">
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
        <div className="col-span-1 lg:col-span-2 space-y-6 text-center md:text-left">
          <img src={LOGO_URL} alt="ToppiLife Logo" className="h-16 mx-auto md:mx-0" />
          <p className="text-gray-500 max-w-md">
            Sứ mệnh của chúng tôi là đem lại giấc ngủ ngon và tinh thần thư thái cho hàng triệu người Việt thông qua công nghệ hiện đại và an toàn nhất.
          </p>
        ...
        <div className="space-y-6">
          <h4 className="font-bold text-gray-900 uppercase tracking-wider">Thông tin liên hệ</h4>
          <ul className="space-y-4 text-gray-600">
            <li className="font-bold text-gray-900">Công ty Cổ phần ToppiGroup</li>
        ...
        <div className="space-y-6">
          <h4 className="font-bold text-gray-900 uppercase tracking-wider">Chứng nhận</h4>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100 flex items-center justify-center">
              <img src="https://upload.wikimedia.org/wikipedia/commons/b/b2/U.S._Food_and_Drug_Administration_logo.svg" alt="FDA" className="h-8 grayscale opacity-50" />
            </div>
            <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100 flex items-center justify-center">
  ...
  <section className="py-24 relative overflow-hidden bg-green-600">
    ...
    <div className="container mx-auto px-4 relative z-10 text-center text-white space-y-8 max-w-4xl">
      ...
      <div className="flex flex-col sm:flex-row gap-6 justify-center pt-4">
        ...
        <a href="https://zalo.me" className="bg-transparent border-2 border-white text-white hover:bg-white/10 px-12 py-5 rounded-full text-xl font-bold transition-all">
          Chat qua Zalo
        </a>
      </div>
    </div>
