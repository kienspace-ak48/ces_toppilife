...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    document.querySelectorAll('#howtouse_cards_wrapper .instance').forEach(el=>{
      let htuCard ={
    ...
    document.querySelectorAll('#howtouse_guide_wrapper .instance').forEach(el=>{
      let htuGuide={
    ...
    //-----
    document.querySelectorAll('#testimonial_wrapper .instance').forEach(el=>{
      let t ={
    ...
    if(document.querySelector('#benefit_footer_solution').value){
      benefitFooterCardSolution = document.querySelector('#benefit_footer_solution').value
      .split('\n')
      .map((i)=>i.trim())
      .filter((i)=>i!=="");
    }
    ...
    if(document.querySelector('#target_user_list')){
      data.target_user_list = document.querySelector('#target_user_list')
      .value.split("\n")
      .map(i=>i.trim())
      .filter(i=>i !=="");
    }
    //convert textarea -> array
    if (data["issue.issues"]) {
      data["issue.issues"] = data["issue.issues"]
        .split("\n")
        .map((i) => i.trim())
        .filter((i) => i !== "");
    }
...
const benefitWrapper = document.querySelector('#benefit_wrapper');
  document.querySelector('#cta_add_benefit_card').addEventListener('click', (ev)=>{
    const clone = document.querySelector('#benefit_template').content.cloneNode(true);
...
const testimonialWrapper = document.querySelector('#testimonial_wrapper');
  document.querySelector('#cta_add_testimonial').addEventListener('click', (ev)=>{
    const clone = document.querySelector('#testimonial_template').content.cloneNode(true);
